...
<form id="item_in" name="item_in" method="post" action="receipt_in_bg.php" onsubmit=" return checkForm()">
    ...
    <fieldset>
        ...
        <p><a href="../test/_self" target="_self"><img src="../image/delete.gif" alt="出入库类型管理" width="25" height="19"
                                                       border="0"/></a></p>
    </fieldset>
    ...
    <fieldset>
        <legend>添加货品</legend>
        <label>编号</label>
        <input id="item_id" name="item_id" type="text" onclick="selectitem()" value="单击选择货品" size="10"
               style="background-color:#CCCCCC" readonly/>
        <label>名称</label>
        ...
        <label>单价</label>
        <input name="item_price" type="text" id="item_price"
               onkeyup="value=value.replace(/[^.\d]/g,'');valueLimit(this)" maxlength="8"/>
        <button type="button" onclick="if(checkInput()==true) addRow();">&nbsp;添加&nbsp;</button>
    </fieldset>
    <fieldset>
        <legend>其它</legend>
        <label>备注</label>
        <textarea name="remark" cols="15" rows="3"
