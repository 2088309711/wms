...
<div style="margin-bottom:8px; height:20px"> 精确查询：
    <input id="receipt_id" name="receipt_id" type="text" value="请输入单据ID" onclick="this.value=''"
           onblur="if(this.value=='') this.value='请输入单据ID'"/>
    <input name="search" type="button" value="查找"
           onclick="url='../receipt_exchange/receipt_show_exchange.php?id=' + document.getElementById('receipt_id').value;window.open(url,'_blank')"/>
</div>
<!--精确查询框架-->
<!--查询框架-->
<div style="margin-bottom:8px; height:20px">
    ...
    <div id="inquiry_time" style="float:left"> 从
        ...
        <input name="search" type="button" value="查找"
               onclick="location.href='inquire_exchange_receipt.php?option=date&date1=' + document.getElementById('date1').value + '&date2=' + document.getElementById('date2').value"/>
    </div>
    <!--日期-->
    <!--仓库-->
