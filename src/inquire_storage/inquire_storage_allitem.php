...
<div>
    ...
    <div style="float:right">筛选：
        <select name="filter" onchange="gotoURL2(this)">
            <option value="0" <?php if ($option == 0) echo 'selected'; ?>>全部</option>
            <option value="1" <?php if ($option == 1) echo 'selected'; ?>>正常</option>
            <option value="2" <?php if ($option == 2) echo 'selected'; ?>>超出上限</option>
            <option value="3" <?php if ($option == 3) echo 'selected'; ?>>低于下限</option>
        </select>
    </div>
    ...
    <div>其它查询方式：</div>
    <div> 按物品查询：
        <input id="item_id" name="item_id" type="text" style="background-color:#CCCCCC" value="请单击选择物品"
               onclick="selectitem()"/>
        <button type="button"
                onclick="if(document.getElementById('item_id').value!='请单击选择物品'){url='inquire_storage_item.php?itemid='+document.getElementById('item_id').value;window.open(url,'_self');}">
            &nbsp;查询&nbsp;
        </button>
